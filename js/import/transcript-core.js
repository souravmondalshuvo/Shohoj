...
export function detectDepartment(text) {
    ...
    if (!compact)
        return null;
    const programMatch = compact.match(/PROGRAM:\s*(.+?)(?=SEMESTER:|COURSE\s+NO|COURSE\s+TITLE|CREDITS\s+EARNED|GRADE\s+POINTS|GRADE\s+SHEET|STUDENT\s+ID|NAME\b|$)/i);
...
export function parseSemesterName(name) {
...
export function parseBlobFallback(text) {
    ...
    if (!semMatches.length)
        return { semesters: [], detectedDept: null };
    const courseRe = /\b([A-Z]{2,4}\d{3}[A-Z]?)\b(.{1,120}?)\b(\d+\.\d+)\s+((?:[A-Z][+-]?)(?:\((?:NT|RT)\))|[A-Z][+-]?)\s+(\d+\.\d+)/g;
    const semesters = semMatches.map((semester, index) => {
...
export function parseTranscriptText(text) {
    ...
    if (totalCourses === 0)
        return legacyParseTranscript(lines, detectedDept);
...
function legacyParseTranscript(lines, detectedDept) {
    const semRe = /^SEMESTER[:\s]*([A-Z]+)\s*(\d{4})\b/i;
