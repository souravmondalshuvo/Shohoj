// Twin of src/core/connectScheduleImport.ts — hand-maintained, not generated.
// src/core/connectScheduleImport.ts is the source of truth: change it there first, then mirror
// the change here. tests/twinParity.test.js fails if the two drift.
//
...
/**
  ...
  * duplicating them.
  */
export function importedSectionId(courseCode, sectionName) {
...
function applyExam(section, kind, date, range) {
    if (kind === 'final') {
...
/**
  ...
  */
export function parseConnectSchedule(text) {
    const warnings = [];
    ...
    const upsert = (courseCode, sectionName, faculties, roomName) => {
        const key = `${courseCode}|${sectionName}`;
    ...
    for (const line of text.split(/\r?\n/)) {
        ...
        if (examDate !== null) {
            ...
            const kind = /\bFINAL\b/i.test(line)
                ? 'final'
                : /\bMID\b/i.test(line)
                    ? 'mid'
                    : null;
    ...
    if (timeless.length > 0) {
        ...
        warnings.push(`Some entries could not be placed on a day: ${[...new Set(unplaced)].join(', ')}.`);
    }
...
/**
  ...
  */
export function picksFromImport(schedule) {
    const picks = {};
