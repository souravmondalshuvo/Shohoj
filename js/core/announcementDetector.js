// Twin of src/features/tasks/detection/announcementDetector.ts — hand-maintained, not generated.
// src/features/tasks/detection/announcementDetector.ts is the source of truth:
// change it there first, then mirror the change here. tests/twinParity.test.js
// fails if the two drift.
//
// Finding academic work in a pasted announcement (#735), for the legacy Tasks
// tab (#767). Deterministic, no clock and no network: `now` arrives in the
// context. THE RULE: an invented deadline is worse than an absent one, so every
// field stays null unless the text actually said it.
//
// Private names carry an _ann prefix against the flattened bundle scope.

import { ALL_COURSES } from './catalog.js';

// The shell's isKnownCourseCode (calculator/catalog.ts) is membership in its
// validated catalogue, which it builds from this same ALL_COURSES: an entry
// counts only with a code, a name and finite credits.
const _ANN_KNOWN_CODES = new Set(
  ALL_COURSES
    .filter((c) => c && typeof c.code === 'string' && c.code.trim() && typeof c.name === 'string' && c.name
      && typeof c.credits === 'number' && Number.isFinite(c.credits))
    .map((c) => c.code.trim().toUpperCase()),
);

function _annIsKnownCourse(code) {
  return _ANN_KNOWN_CODES.has(String(code ?? '').trim().toUpperCase());
}
...
const _ANN_COURSE_RE = /\b([A-Z]{2,4})\s?-?\s?(\d{3}[A-Z]?)\b/g;
...
const _ANN_MONTH_ALTS = Object.keys(_ANN_MONTHS).join('|');

/** "25 September" / "25 Sep 2026" / "25th of September" */
const _ANN_DAY_MONTH = new RegExp(
  `\\b(\\d{1,2})(?:st|nd|rd|th)?\\s+(?:of\\s+)?(${_ANN_MONTH_ALTS})\\.?(?:\\s+(\\d{4}))?\\b`,
  'i',
);
/** "September 25" / "Sep 25, 2026" */
const _ANN_MONTH_DAY = new RegExp(
  `\\b(${_ANN_MONTH_ALTS})\\.?\\s+(\\d{1,2})(?:st|nd|rd|th)?(?:,?\\s+(\\d{4}))?\\b`,
  'i',
);
/** "25/09/2026" or "25-09-26" — day first, which is how Bangladesh writes it. */
const _ANN_NUMERIC_DATE = /\b(\d{1,2})[/-](\d{1,2})(?:[/-](\d{2,4}))?\b/;
/** "at 9:30 pm" / "9 am" / "14:00" */
const _ANN_TIME_RE = /\b(?:at\s+)?(\d{1,2})(?::(\d{2}))?\s*(am|pm)?\b/i;
/** "chapters 4-6", "ch 4 to 6", "topics 1-3" */
const _ANN_SYLLABUS_RE = /\b((?:chapters?|ch|topics?|units?|lectures?)\.?\s*\d+\s*(?:[-–—]|to)\s*\d+)/i;
/** A single chapter: "chapter 7" */
const _ANN_SYLLABUS_ONE_RE = /\b((?:chapters?|ch|topics?|units?|lectures?)\.?\s*\d+)/i;

function _annEvidence(text, index) {
 ...
 */
function _annResolveYear(month, day, now) {
...
function _annFindDate(text, now) {
  let day = null;
  let month = null;
  let year = null;
  let source = null;

  const dayMonth = _ANN_DAY_MONTH.exec(text);
  const monthDay = _ANN_MONTH_DAY.exec(text);
  const numeric = _ANN_NUMERIC_DATE.exec(text);

  if (dayMonth) {
    day = Number(dayMonth[1]);
    month = _ANN_MONTHS[(dayMonth[2] ?? '').toLowerCase()] ?? null;
    year = dayMonth[3] ? Number(dayMonth[3]) : null;
    source = { text: dayMonth[0], index: dayMonth.index };
  } else if (monthDay) {
    month = _ANN_MONTHS[(monthDay[1] ?? '').toLowerCase()] ?? null;
  ...
  const resolvedYear = year ?? _annResolveYear(month, day, now);
...
function _annFindCourse(text, known) {
  const upper = text.toUpperCase();
  const seen = [];
  _ANN_COURSE_RE.lastIndex = 0;
  let match;
  while ((match = _ANN_COURSE_RE.exec(upper)) !== null) {
  ...
  const real = seen.find((c) => _annIsKnownCourse(c.code));
  const chosen = enrolled ?? real;
  if (chosen === undefined) return null;
  return { code: chosen.code, evidence: _annEvidence(chosen.code, chosen.index) };
}
 ...
 */
function _annBuildTitle(text, type, typeMatch) {
  ...
  const numbered = new RegExp(
    `\\b${_annEscapeRegex(typeMatch)}\\s*(?:no\\.?\\s*)?(\\d{1,2})\\b` +
      `(?![/-]\\d|\\s*(?:st|nd|rd|th)?\\s+(?:of\\s+)?(?:${_ANN_MONTH_ALTS}))`,
    'i',
  ).exec(text);
...
function _annEscapeRegex(value) {
 ...
 */
function _annRateConfidence(hasDate, hasCourse, hadTime) {
...
function _annDetectOne(line, context) {
  const typeHit = _ANN_TYPE_WORDS.find(([pattern]) => pattern.test(line));
  ...
  const date = _annFindDate(line, context.now);
  const course = _annFindCourse(line, context.knownCourseCodes ?? []);
  const syllabusMatch = _ANN_SYLLABUS_RE.exec(line) ?? _ANN_SYLLABUS_ONE_RE.exec(line);

  const ev = {};
  if (typeWord !== '') ev['type'] = _annEvidence(typeWord, matched?.index ?? 0);
  if (date !== null) ev['dueAt'] = date.evidence;
  if (course !== null) ev['courseCode'] = course.evidence;
  if (syllabusMatch?.[1] !== undefined) {
    ev['syllabus'] = _annEvidence(syllabusMatch[1], syllabusMatch.index);
  }

  return {
    title: _annBuildTitle(line, type, typeWord),
    type,
    dueAt: date?.iso ?? null,
    courseCode: course?.code ?? null,
    syllabus: syllabusMatch?.[1]?.trim() ?? null,
    confidence: _annRateConfidence(date !== null, course !== null, date?.hadTime ?? false),
    evidence: ev,
  };
}

/** The pasted-announcement detector. */
export const announcementDetector = {
  source: 'PASTE',
  detect(input, context) {
    const lines = _annCandidates(input);
    const detected = [];
    const unrecognised = [];

    for (const line of lines) {
      const found = _annDetectOne(line, context);
...
export function detectFromText(input, context) {
  return announcementDetector.detect(input, context);
}
