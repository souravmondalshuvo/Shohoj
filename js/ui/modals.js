...
export function exportPDF() {
  ...
  const gpaBg = gpa =>
    gpa >= 3.0 ? GREEN_BG : gpa >= 2.5 ? AMBER_BG : RED_BG;
  ...
  // Logo mark
  setFill(GREEN);
  doc.roundedRect(ML, y + 4, 18, 18, 2.5, 2.5, 'F');
  ...
  // Divider line
  setStroke(GREEN); doc.setLineWidth(0.8);
  ...
  stats.forEach(([val, label], i) => {
    ...
    const valCol = label === 'Academic Standing'
      ? (cgpa >= 3.0 ? GREEN_DARK : cgpa >= 2.5 ? AMBER_TXT : RED_TXT)
      : TEXT1;
  ...
  // ── SEMESTER SECTIONS ──────────────────────────────────────────────────────
  // Column positions: name=ML, cr right@ML+120, gp right@ML+135, grade@ML+139(badge 24mm→163), note@ML+166
  ...
  state.semesters.forEach(sem => {
    ...
    doc.rect(ML + 1.5, y, 1.5, 8, 'F'); // square off right side of accent

    // Sem name — strip HTML tags, replace ordinal sup
    const semNameClean = sem.name
      .replace(/<[^>]+>/g, '')
      .replace(/\s*\((\d+(?:st|nd|rd|th)) Semester\)/i, ' | $1 Semester')
      + (sem.running ? ' [Running]' : '');
    ...
    if (semGpa !== null) {
      ...
      // GPA pill (right-aligned)
      const pillW = 22, pillH = 5, pillX = PW - MR - pillW;
      setFill(gBg); setStroke(gCol.map ? gCol : gCol);
      doc.setDrawColor(gCol[0], gCol[1], gCol[2]);
    ...
    sem.courses.forEach((c, ci) => {
      ...
      // Alternating row bg
      if (ci % 2 === 0) {
        setFill(LGREY); doc.rect(ML, y - 0.5, CW, 7, 'F');
      }
      ...
      doc.text(
        c.credits !== undefined && c.credits !== null ? c.credits.toString() : '--',
        COL.cr, y + 4.5, { align: 'right' }
      );
  ...
  // Footer logo dot
  setFill(GREEN);
  doc.roundedRect(ML, footerY + 2.5, 7, 7, 1.5, 1.5, 'F');
