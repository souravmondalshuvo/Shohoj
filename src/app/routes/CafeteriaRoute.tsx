...
function hoursSummary(outlet: CafeteriaOutlet, todayIndex: number): { day: string; text: string; today: boolean }[] {
    ...
    return (
        <section className="shell-page cafeteria-page" data-testid="cafeteria-page">
            ...
            <p className="shell-muted">
                Where to eat on campus — each outlet&apos;s location, opening hours and
                how to pay. No daily menus, by design.
            </p>
                    ...
                    >
                        <div className="cafeteria-card-head">
                            <h2 className="cafeteria-name">
                                {outlet.name}{' '}
                                <span className="cafeteria-kind">{outletKindLabel(outlet.kind)}</span>
                            </h2>
                            <span
                                ...
                                <tbody>
                                    {hoursSummary(outlet, todayIndex).map((row) => (
                                        <tr key={row.day} className={row.today ? 'cafeteria-hours--today' : undefined}>
                                            <th scope="row">{row.day}{row.today ? ' (today)' : ''}</th>
                                            <td>{row.text}</td>
                                        </tr>
                                    ))}
                                </tbody>
                        ...
                        <p className="cafeteria-pay">
                            Payment: {outlet.payment.join(' · ')}
                        </p>
