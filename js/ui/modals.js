...
import { getRetakenKeys } from './render.js';
...
function exportPDF() {
      ...
      state.semesters.forEach(sem => {
        ...
        y += 4; // gap between state.semesters
      });
...
// ── MODAL THEME ───────────────────────────────────────
    ...
    async function importTranscriptPDF(inputEl) {
      ...
      try {
        ...
        // ── PARSE ──────────────────────────────────────
        let parsed = app.parseTranscriptText(fullText);

        // ── FALLBACK: blob parser ─────────────────────
        const totalCourses = parsed.state.semesters.reduce((s,sem)=>s+sem.courses.length,0);
        const semCount     = parsed.state.semesters.length;
        if (semCount > 0 && totalCourses < semCount * 2) {
          ...
          if (blobParsed.state.semesters.length > 0) {
            const blobTotal = blobParsed.state.semesters.reduce((s,sem)=>s+sem.courses.length,0);
        ...
        if (!parsed.state.semesters.length) {
          showImportModal(`
            <div style="text-align:center;padding:20px">
              ...
              <div style="font-size:13px;color:${getModalTheme().text2};margin-bottom:16px">
                No state.semesters were detected. Make sure this is a BRACU official grade sheet PDF.
              </div>
        ...
        const semRows = parsed.state.semesters.map(s => `
        ...
        const totalCoursesDisplay = parsed.state.semesters.reduce((n, s) => n + s.courses.length, 0);

        showImportModal(`
          <div style="margin-bottom:16px">
            <div style="font-size:18px;font-weight:700;color:${t2.text};margin-bottom:4px">📄 Transcript Parsed</div>
            <div style="font-size:12px;color:${t2.text3}">Found <strong style="color:#1DB954">${parsed.state.semesters.length} state.semesters</strong> and <strong style="color:#1DB954">${totalCoursesDisplay} courses</strong></div>
          </div>
          ...
          <div style="display:flex;gap:10px">
            <button onclick="app.applyImport(${JSON.stringify(parsed).replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;')})"
...
export { exportPDF, getModalTheme, showImportModal, hideImportModal, importTranscriptPDF };