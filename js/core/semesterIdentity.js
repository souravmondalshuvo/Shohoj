// Twin of src/core/semesterIdentity.ts — hand-maintained, not generated.
// src/core/semesterIdentity.ts is the source of truth: change it there first, then mirror the
...
// would drift between this file and its src/ twin.
 ...
 */
export const SEMESTER_TERM_NAMES = Object.freeze({
    1: 'Spring',
    2: 'Summer',
    3: 'Fall',
});
...
export function formatSemesterDate(iso) {
...
export function semesterNameFromSessionId(sessionId) {
...
function classify(todayISO, startDate, endDate) {
 ...
 */
export function describeSemester(sections, todayISO) {
    const list = Array.isArray(sections) ? sections : [];
    const sessionId = modalValue(
        list.map((s) => s.semesterSessionId).filter((v) => typeof v === 'number'),
    );
    const classStartDate = modalValue(
        list.map((s) => s.classStartDate).filter((v) => typeof v === 'string'),
    );
    const classEndDate = modalValue(
        list.map((s) => s.classEndDate).filter((v) => typeof v === 'string'),
    );
 ...
 */
export function semesterIsRunning(identity) {
 ...
 */
export function semesterCaveat(identity) {
...
export function semesterHeadline(identity) {
