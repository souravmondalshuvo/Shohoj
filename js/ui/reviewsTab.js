...
async function _renderDeptList(root, token) {
  ...
  const updateSuggestions = () => {
    ...
    if (!courses.length && !faculty.length) {
      hideSuggestions();
      return;
    }
  ...
  root.innerHTML = `
    <div class="rv-tab">
      <div class="rv-tab-breadcrumb">
        ...
        <span class="rv-tab-crumb-active">${escHtml(meta?.displayCode || dept)}</span>
      </div>
      ...
      <div id="_rvt_coursebody">
        ${_courseGridSkeletonHtml(skeletonCount)}
      </div>
    </div>`;
  ...
  const courseCardHtml = (code, i) => {
    ...
    return `
      <div class="rv-tab-coursecard" data-course="${escAttr(code)}" role="button" tabindex="0" style="--i:${Math.min(i, 24)}">
        ...
        ${count > 0
          ? `<div class="rv-tab-coursecard-count rv-tab-coursecard-count--reviews">${facCount} faculty · ${count} review${count !== 1 ? 's' : ''}</div>`
...
function _courseGridSkeletonHtml(count = 12) {
  const cards = Array.from({ length: count }, () => `
    <div class="rv-tab-coursecard rv-tab-coursecard--skeleton">
      <div class="rv-tab-skel rv-tab-skel-course-code"></div>
      <div class="rv-tab-skel rv-tab-skel-course-name"></div>
      <div class="rv-tab-skel rv-tab-skel-course-name rv-tab-skel-course-name--short"></div>
      <div class="rv-tab-skel rv-tab-skel-course-count"></div>
    </div>`).join('');

  return `
    <div class="rv-tab-coursegrid rv-tab-coursegrid--skeleton" aria-label="Loading courses">
      ${cards}
    </div>`;
}
  ...
  root.innerHTML = `
    <div class="rv-tab">
      <div class="rv-tab-breadcrumb">
        ...
        ${deptMeta ? `
          <span class="rv-tab-crumb-sep">›</span>
          <a href="#calculator/reviews/dept/${escAttr(deptCode)}" class="rv-tab-crumb">${escHtml(deptMeta.displayCode || deptCode)}</a>` : ''}
        <span class="rv-tab-crumb-sep">›</span>
...
async function _renderFacultyPage(root, initials, courseFilter, token) {
  ...
  const gmailLink = facultyEmail
    ? `https://mail.google.com/mail/?view=cm&to=${encodeURIComponent(facultyEmail)}`
    : '';
  ...
  const overviewHtml = overview ? `
    <div class="rv-tab-overview">
      <div class="rv-tab-overview-label"><span class="rv-tab-overview-dot"></span>Overall Overview</div>
  ...
  const aggHtml = showAgg ? `
    <div class="rv-tab-aggcard">
      ...
      <div class="rv-tab-dimgroup">
        ...
        <div class="rv-tab-aggcard-grid">
          ${_dimCell('Difficulty', r.difficulty, true)}
          ${_dimCell('Workload',   r.workload,   true)}
        </div>
      </div>
      ${overviewHtml}
...
function _facultyDirectoryRowHtml(g, courseScope = '') {
  ...
  const badgeClass = knownOnly
    ? 'rv-tab-directory-badge'
    : (hasAgg ? 'rv-tab-directory-badge rv-tab-directory-badge--green' : 'rv-tab-directory-badge rv-tab-directory-badge--gold');
  ...
  return `
    <div class="rv-tab-directory-row" data-faculty="${escAttr(g.facultyInitials)}" data-course="${escAttr(courseScope)}" role="button" tabindex="0">
      ...
      <div class="rv-tab-directory-side">
        <span class="${escAttr(badgeClass)}">${escHtml(g.facultyInitials)}</span>
        ${knownOnly
...
function _dimScoreClass(value) {
  if (typeof value !== 'number') return '';
  if (value >= 4.2) return 'rv-tab-dimcell-value--great';
  if (value >= 3.5) return 'rv-tab-dimcell-value--good';
  if (value >= 2.8) return 'rv-tab-dimcell-value--mid';
  if (value >= 2.0) return 'rv-tab-dimcell-value--warn';
  return 'rv-tab-dimcell-value--bad';
}

function _dimScoreClassInverse(value) {
  if (typeof value !== 'number') return '';
  if (value <= 1.8) return 'rv-tab-dimcell-value--great';
  if (value <= 2.5) return 'rv-tab-dimcell-value--good';
  if (value <= 3.2) return 'rv-tab-dimcell-value--mid';
  if (value <= 4.0) return 'rv-tab-dimcell-value--warn';
  return 'rv-tab-dimcell-value--bad';
}

function _dimCell(label, value, inverse = false) {
  const v   = (typeof value === 'number') ? value.toFixed(1) : '—';
  const cls = inverse ? _dimScoreClassInverse(value) : _dimScoreClass(value);
  return `
    <div class="rv-tab-dimcell">
      <div class="rv-tab-dimcell-label">${label}</div>
      <div class="rv-tab-dimcell-value ${cls}">${v}</div>
    </div>`;
}
