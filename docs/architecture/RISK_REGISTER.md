...
> Date: 2026-06-28. Severity × Probability → Priority. Reassess each phase.
...
| R9 | **npm lock drift on dep bumps** — incremental install corrupts package-lock (@rolldown/@emnapi) → `npm ci` fails | M | M | P3 | Clean reinstall + `npm ci --dry-run`; bump deps manually; Dependabot intentionally off | all |
